...
**NOTE:** When mods are supported in multiplayer, the game will include an API for storing data on actors. When that is added, this tutorial will be updated. The API we design is quite similar to the one the game will provide, so you won't have to change much when it's eventually added.
...
public TutorialPlayerData GetData(ITMPlayer player)
{
    ...
    data = new TutorialPlayerData(_game, player);
...
First, we'll replace our random number with a float property called "Mana," and set it to 100 by default. We'll also add an `Update` method that regenerates `5 / Services.ElapsedTime` mana every frame, up to 100.
...
**IMPORTANT:** *Make sure this is called before we initialize our player data, but after we create the TutorialPlayerDataManager!*
...
You can find the code for this stage of the project [here](https://github.com/DaveTheMonitor/TMModTutorial/tree/master/PlayerData). Feel free to cross-check your project with this one to ensure you didn't miss anything.
